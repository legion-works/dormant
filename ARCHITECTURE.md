...
- **Wire events** (`DaemonEvent` in `dormant-core/src/rules.rs`): tag is `event` — `sensor_changed`, `zone_changed`, `display_phase`, `pause_changed`, `config_reloaded`, `wake_retry`, `wake_recovered`, `blank_failure`, `blank_recovered`, `wear_snapshot`, `compensation_advisory`, `ownership`. Pairing status is a request/response IPC and loopback-HTTP contract, not a daemon event. A `DaemonEvent::Subscribed` sentinel is the first frame emitted on a fresh event-stream connection (per-connection, never broadcast).
