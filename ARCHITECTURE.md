...
| `dormantctl` | CLI binary: `status`, `pause`, `resume`, `blank`, `wake`, `reload`, `validate`, `watch`, `emergency-wake` (IPC fast path with a direct-hardware fallback when the daemon is unresponsive — also runs the macOS gamma breadcrumb restore independently of daemon health), `doctor` (per-target subcommands including `samsung` and the macOS-only read-only arms, plus `--exercise <display>` for control-path verification), `pair samsung <host>`, `launchd install`/`launchd uninstall` (macOS-only handler — installs/removes the checked-in daemon and tray `LaunchAgent` plists at their canonical paths under `~/Library/LaunchAgents/`, `cmd_launchd.rs`) subcommands. Also re-exports its IPC `client` module as a library entry (`crates/dormantctl/src/lib.rs`) so `dormant-tray` can drive the same protocol without forking the socket glue | **Yes** — `dormantctl` |
...
- **Wire events** (`DaemonEvent` in `dormant-core/src/rules.rs`): tag is `event` — `sensor_changed`, `zone_changed`, `display_phase`, `config_reloaded`, `wake_retry`, `wake_recovered`, `blank_failure`, `blank_recovered`, `wear_snapshot`, `compensation_advisory`. A `DaemonEvent::Subscribed` sentinel is the first frame emitted on a fresh event-stream connection (per-connection, never broadcast) — it marks that the daemon-side broadcast receiver is registered, so events that fire after the line is written are guaranteed to be delivered; the tray swallows it, `connect_events` strips it from the iterator so callers never see it. A `#[serde(other)]` `Unknown` variant keeps older CLIs/WebUI builds streaming past foreign tags rather than failing the iterator (`crates/dormantctl/src/client.rs::EventStream` round-trips them through `DaemonEvent::Unknown`).
