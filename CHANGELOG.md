...
- Micro pixel-shift: the dormant overlay surfaces (black and screensaver) now raster-walk their `wp_viewport` source rect by `shift_px` buffer pixels every `shift_interval` (defaults `2` / `120s`) to even out static-image burn-in, per the live-probe-validated mechanism. **Behavior change:** any display that already has a `[displays.<id>.screensaver]` table gets the shift enabled by default on upgrade — set `shift_px = 0` to opt out. Displays without a screensaver table are unaffected. Note this extends the original spec (screensaver-only, U5) to the black overlay as well, by operator decision alongside the probe findings.
