...
- Micro pixel-shift: the screensaver overlay surface now raster-walks its `wp_viewport` source rect by `shift_px` buffer pixels every `shift_interval` (defaults `2` / `120s`) to even out static-image burn-in, per the live-probe-validated mechanism. Per spec invariant U5 the black overlay never shifts — a uniform RGB(0,0,0) field is translation-invariant on OLED (subpixels off), so shifting buys zero wear protection and costs an oversized shm buffer (~21 MB at 3072×1728) plus periodic damage/commit wakeups, replacing the near-free `wp_single_pixel_buffer` fast path. **Behavior change:** any display that already has a `[displays.<id>.screensaver]` table gets the shift enabled by default on upgrade — set `shift_px = 0` to opt out. Displays without a screensaver table are unaffected.
