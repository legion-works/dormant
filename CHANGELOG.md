...
- Linux release binaries are built on Ubuntu 24.04 rather than 22.04, because active wear sampling links against PipeWire 1.0 headers that 22.04 does not ship. Prebuilt Linux binaries now require glibc 2.39 or newer, and PipeWire's client library at runtime (`libpipewire-0.3-0` on Debian and Ubuntu, `pipewire` on Fedora and Arch). On an older distribution, build from source or install from the AUR, which compile against your own system libraries.
