...
- **Display controllers** — `ddcci` (VCP `0xD6` power-off and brightness-zero), `samsung-tizen` (`KEY_PICTURE_OFF`, power-key, Wake-on-LAN, network pairing), `kwin-dpms`, `ha-passthrough`, and a generic `command` escape hatch. Per-display fallback chain with bounded wake-retry.
- **Audio-safe blanking** — DDC/CI `0xD6` on capable monitors and Tizen picture-off on the TV both blank the panel without tearing down the output, so audio survives. DPMS is the documented fallback only where there is no audio to lose.
