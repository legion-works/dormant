...
"#;
    ...
    /// M4 regression: the reviewer's exact failing JSON uses
    /// `["displays","shared_oled","hooks","on_observed_loss"]` — prove the
    /// `KNOWN_KEYS` entry for the fifth slot works when the flag is enabled.
    #[tokio::test]
    async fn on_observed_loss_path_not_blocked_by_flag_when_hook_edit_enabled() {
        let dir = tempfile::tempdir().unwrap();
        let content = r#"
config_version = 1
...
[displays.shared_oled.hooks]
on_observed_loss = [{ command = ["/usr/bin/example-hook"] }]
"#;
        write_config(dir.path(), content);
        ...
        if let Err(e) = result {
            ...
            assert_ne!(
                resp.status(),
                StatusCode::FORBIDDEN,
                "on_observed_loss Set must NOT be denied by the flag gate when hook_edit_enabled is true"
            );
        }
    }

    #[tokio::test]
    async fn on_observed_gain_path_not_blocked_by_flag_when_hook_edit_enabled() {
        let dir = tempfile::tempdir().unwrap();
        let content = r#"
config_version = 1
[daemon]
...
shared_input_code = 0x0f
[displays.shared_oled.hooks]
on_observed_gain = [{ command = ["/usr/bin/example-hook"] }]
"#;
        write_config(dir.path(), content);
        let mut cfg = minimal_config();
        cfg.daemon.hook_edit_enabled = true;
        ...
        if let Err(error) = post_apply(State(state), axum::Json(req)).await {
            assert_ne!(
                ...
                StatusCode::FORBIDDEN,
                "on_observed_gain Set must not be denied when hook editing is enabled"
            );
        }
    }
