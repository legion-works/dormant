    ...
    <div className="cf-card cf-card--create" data-testid={`create-${collection}-form`}>
      ...
      {collection === "displays" && (
        <>
          ...
          {displayScope === "shared" && (
            <>
              <HexCodeField path={p("shared_input_code")} label="shared_input_code" value={fields.shared_input_code ?? ""} locked={false} onEdit={(_p, v) => setField("shared_input_code", v)} />
              <HexCodeField path={p("shared_input_write_code")} label="shared_input_write_code (optional)" value={fields.shared_input_write_code ?? ""} locked={false} onEdit={(_p, v) => setField("shared_input_write_code", v)} />
              <HexCodeField path={p("shared_peer_input_code")} label="shared_peer_input_code (optional peer read code)" value={fields.shared_peer_input_code ?? ""} locked={false} onEdit={(_p, v) => setField("shared_peer_input_code", v)} />
              <HexCodeField path={p("shared_peer_input_write_code")} label="shared_peer_input_write_code (optional peer write code)" value={fields.shared_peer_input_write_code ?? ""} locked={false} onEdit={(_p, v) => setField("shared_peer_input_write_code", v)} />
            </>
          )}
