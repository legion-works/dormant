...
#[cfg(feature = "render")]
/// Split ordered render reports into contiguous intervals within one
/// monotonic attribution window. The caller supplies the retained report from
/// the previous drain as part of `reports`; no journal or tracker state is
/// consulted here.
fn split_exposure_slices(
    reports: &[ScreensaverItemReport],
    start: Instant,
    boundary: Instant,
) -> Option<Vec<ScreensaverExposureSlice>> {
    let mut current = reports
        .iter()
...
fn collect_exposure_slices(
    ...
    let default_span = Duration::from_secs(sample_interval_s);
    let elapsed = state.screensaver_tick_at.map_or(default_span, |previous| {
        boundary
            .checked_duration_since(previous)
            .unwrap_or_default()
    });
    let span = elapsed.min(Duration::from_secs(max_span_s));
    let start = boundary.checked_sub(span).unwrap_or(boundary);
    state.screensaver_tick_at = Some(boundary);
    let mut output = HashMap::new();

    for display_id in state.ledgers.keys() {
        ...
        let Ok(mut guard) = journal.lock() else {
            ...
            output.insert(
                display_id.clone(),
                vec![ScreensaverExposureSlice { span, luma: None }],
            );
            continue;
        };
        ...
        let slices = split_exposure_slices(&reports, start, boundary)
            .unwrap_or_else(|| vec![ScreensaverExposureSlice { span, luma: None }]);
...
struct TrackerState {
    ...
    /// Monotonic boundary of the previous screensaver attribution tick.
    #[cfg(feature = "render")]
...
#[derive(Debug, Clone, PartialEq)]
enum TrackerAction {
    /// A fresh active-time grid survived shell filtering and phase re-check.
    /// Task 13 turns this selection into spatial ledger attribution.
    SampledSelection {
        display: DisplayId,
        sample: SampledGrid,
    },
    /// No fresh grid is available for an active-time attribution window.
    UniformSelection {
        display: DisplayId,
...
pub enum SampleFallbackTag {
    /// The sampler has not published a grid for this tick.
    Missing,
...
enum AttributionSelection<'a> {
    Sampled(&'a SampledGrid),
    Uniform(Option<SampleFallbackTag>),
}

/// Keep capture-age arithmetic at the shell boundary with its monotonic tick.
fn filter_sample_for_tick(
    sample: Option<&SampledGrid>,
    boundary: Tick,
    max_age: Duration,
) -> (Option<&SampledGrid>, Option<SampleFallbackTag>) {
    let Some(sample) = sample else {
        return (None, Some(SampleFallbackTag::Missing));
    };
    let age = boundary.0.saturating_duration_since(sample.captured_at.0);
    if age <= max_age {
        (Some(sample), None)
    } else {
...
fn select_sample_for_attribution<'a>(
    phase: &str,
    stage: Option<&StageInfo>,
    sampled: Option<&'a SampledGrid>,
    fallback: Option<SampleFallbackTag>,
) -> AttributionSelection<'a> {
    if stage.is_none() && matches!(phase, "active" | "grace") {
        sampled.map_or(
            AttributionSelection::Uniform(fallback),
            AttributionSelection::Sampled,
        )
    } else {
        AttributionSelection::Uniform(None)
    }
}
                    ...
                    && cfg.active_sampling.sampled_display.as_deref() == Some(display_id.0.as_str())
                {
                    ...
                    match selection {
                        AttributionSelection::Sampled(sample) => {
                            actions.push(TrackerAction::SampledSelection {
                                display: display_id.clone(),
                                sample: sample.clone(),
                            });
...
mod tests {
    ...
    #[test]
    fn sampled_active_and_grace_select_sampled_over_uniform() {
        let boundary = Tick::now();
        let sample = SampledGrid {
            grid: LumaGrid::new(vec![0.5; 9 * 16]).unwrap(),
            captured_at: boundary,
            phase_at_capture: dormant_core::state_machine::Phase::Active,
        };

        let (fresh, fallback) =
            filter_sample_for_tick(Some(&sample), boundary, Duration::from_secs(120));

        assert!(fallback.is_none());
        for phase in ["active", "grace"] {
            let display = DisplayId(phase.into());
            let mut state = TrackerState::default();
            state
                .ledgers
                .insert(display.clone(), fresh_ledger(&display, 0));
            ...
            let actions = tick(
                &mut state,
                &snapshot_with(&display, phase, None),
                &HashMap::new(),
                &cfg,
                u64::MAX,
                fresh,
            ...
            assert!(actions.iter().any(|action| matches!(
                action,
                TrackerAction::SampledSelection { display: selected, sample: selected_sample }
    ...
    #[test]
    fn filter_sample_for_tick_keeps_exactly_two_intervals_old_sample() {
        let boundary = Tick::now();
        let max_age = Duration::from_secs(120);
        let sample = SampledGrid {
            grid: LumaGrid::new(vec![0.5; 9 * 16]).unwrap(),
            captured_at: Tick(boundary.0.checked_sub(max_age).unwrap()),
            phase_at_capture: dormant_core::state_machine::Phase::Active,
        };

        let (fresh, fallback) = filter_sample_for_tick(Some(&sample), boundary, max_age);
    ...
    #[test]
    fn filter_sample_for_tick_marks_older_than_two_intervals_stale() {
        let boundary = Tick::now();
        let max_age = Duration::from_secs(120);
        let sample = SampledGrid {
            grid: LumaGrid::new(vec![0.5; 9 * 16]).unwrap(),
        ...
        assert_eq!(fallback, Some(SampleFallbackTag::Stale));
    }

    #[test]
    fn phase_recheck_discards_active_capture_outside_active_or_grace_without_stage() {
        let boundary = Tick::now();
        let sample = SampledGrid {
            grid: LumaGrid::new(vec![0.5; 9 * 16]).unwrap(),
            captured_at: boundary,
        ...
        for phase in ["blanked", "staged", "waking"] {
            assert!(matches!(
                select_sample_for_attribution(phase, None, Some(&sample), None),
                AttributionSelection::Uniform(_)
            ));
        }
        assert!(matches!(
            select_sample_for_attribution(
                "active",
                Some(&StageInfo {
                    idx: 0,
    ...
    #[test]
    fn sampled_missing_suspended_state_routes_uniform_with_sampler_literal() {
        let display = DisplayId("mon".into());
        let mut state = TrackerState::default();
        state
            .ledgers
        ...
        assert!(actions.iter().any(|action| matches!(
            ...
            TrackerAction::UniformSelection {
                display: selected,
                fallback: Some(SampleFallbackTag::Missing),
            } if selected == &display
        )));
        assert_eq!(
            crate::active_sampler::WEAR_SAMPLING_SUSPENDED,
            "wear_sampling_suspended"
        );
    }
    ...
    #[test]
    fn staged_screensaver_uses_monotonic_window_when_wall_clock_diverges() {
        let display = DisplayId("mon".into());
        let mut ledger = fresh_ledger(&display, 0);
        ledger.last_sample_at_epoch_s = Some(1_000_000);
        let mut state = TrackerState::default();
        ...
        // The injected wall clock advanced by two minutes, while the
        // monotonic exposure window contains only one minute.
        let _ = tick(
            &mut state,
            &snapshot,
            &HashMap::new(),
        ...
        assert!(
            (state.ledgers[&display].cells[0].wear_hours - cfg.screensaver_factor / 60.0).abs()
                < 1e-12
        );
    }
    ...
    fn staged_screensaver_uses_pre_window_item_for_leading_dwell() {
        ...
        let exposures =
            collect_exposure_slices(&journal, &mut state, boundary, &WearConfig::default());
    ...
    #[cfg(feature = "render")]
    #[test]
    fn pure_exposure_splitter_uses_entries_and_monotonic_window_only() {
        let display = DisplayId("mon".into());
        let boundary = Instant::now() + Duration::from_secs(60);
        let start = boundary.checked_sub(Duration::from_secs(60)).unwrap();
        let reports = vec![
        ...
        let slices = split_exposure_slices(&reports, start, boundary).unwrap();
        assert_eq!(
            slices.iter().map(|slice| slice.span).collect::<Vec<_>>(),
            [Duration::from_secs(30), Duration::from_secs(30),]
        );
    ...
    fn staged_screensaver_overflow_without_window_start_degrades_to_uniform_factor() {
        ...
        let exposures =
            collect_exposure_slices(&journal, &mut state, boundary, &WearConfig::default());
        let fallback = exposures.get(&display).expect("incomplete window marker");
        assert_eq!(fallback.len(), 1);
        assert_eq!(fallback[0].span, Duration::from_secs(60));
        assert!(fallback[0].luma.is_none());
