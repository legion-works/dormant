...
- **VCP `0xB6` (Display Type) → reports `LCD (active matrix)` (sl=0x03) — an MCCS-convention artifact, NOT the truth.** The AG326UZD is a **QD-OLED** (confirmed by the device owner + web sources 2026-07-09). Reporting OLED panels as "LCD" through `0xB6` is a known MCCS firmware convention, not a per-unit bug. HARD RULE: never use `0xB6` for panel-type detection — use EDID / model lookup / config override (`0xB2` sub-pixel layout, `0xC8` controller type are adjacent hints). Because it's QD-OLED, the ledger's `blue_wear_factor` = 1.0 (white stresses all subpixels equally; a WOLED panel would be ~1.2–1.5). ✓ resolved.
