...
| `flap_threshold` | integer | `8` | Committed ownership transitions allowed inside `flap_window` before the panel becomes contested; minimum `2`. Eight permits four human KVM round-trips in two minutes, while a 6–10s standby flap reaches 12–20 transitions and is contained within about 90 seconds. |
