...
| `flap_settle` | duration | `"60s"` | Quiet interval without a committed transition before a contested panel resumes normal evaluation; must be `>= poll_interval`. While contested, the daemon forces not-owned rather than risking a power-off on a panel the peer is using. |
