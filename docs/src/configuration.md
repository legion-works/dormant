...
| `circuit_reset_after` | duration | `"5m"` | Open-circuit retry delay |
...
| `flap_threshold` | integer | `5` | Committed ownership transitions allowed inside `flap_window` before the panel becomes contested; minimum `2`. Five keeps three human KVM round-trips over five minutes below the limit in any two-minute window while containing live standby readback bursts reaching six committed transitions per window. |
| `flap_window` | duration | `"120s"` | Sliding interval for `flap_threshold`; must be `>= poll_interval`. |
| `flap_settle` | duration | `"60s"` | Quiet time — no change in the panel's reported input — before a contested panel resumes normal evaluation; must be `>= poll_interval`. While contested, the daemon forces not-owned rather than risking a power-off on a panel the peer is using. |
| `reprobe_failure_threshold` | integer | `3` | Consecutive failed input-source reads before the daemon attempts on-demand DDC/controller healing; range `1..=10`. |
| `reprobe_interval` | duration | `"30s"` | Floor between healing attempts after sustained read failures; must be `>= poll_interval` and `<= "120s"`. Failed attempts use bounded backoff (`30s → 60s → 120s`), capped at `120s`; a successful read resets the next delay to this floor while retaining the last-attempt timestamp so flap boundaries remain rate-limited. |
