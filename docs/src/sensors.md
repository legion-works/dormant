...
Ready-to-flash ESPHome configs for the MQTT topology live in [`examples/esphome/`](https://github.com/legion-works/dormant/tree/dev/examples/esphome): start with `02-standard-desk.yaml` for near/far distance zones, per-gate sensitivity thresholds (g0–g8 move/still), an energy-gated `desk_seated` template that bypasses the LD2410C's presence-flag quirks, and a tunable still-energy floor. The USB topology needs only a `type = "usb-ld2410"` sensor block (see below) and a ~$2 adapter.
