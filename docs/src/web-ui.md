...
The web UI is **intended for single-user, loopback-only use**. There is no authentication, no user sessions, and no TLS. The design assumes the operator runs `dormantd` on their local machine and opens the dashboard from the same host. Entity CRUD and the pairing wizard (below) are built on this same floor — they add attack surface (a second credentials-writing route, a route that can mint new config entities), but the floor itself is unchanged from what the config editor shipped under.
...
- **Locked leaves** — `type` on an *existing* entity, `blank_data`, `wake_data`: never writable through the patch API. The form renders these with a 🔒 icon and a tooltip explaining the restriction (redacted path ancestor, or a hard-locked config key). `type` is settable only once, at entity creation (see [Entity create/delete](#entity-createdelete) below) — changing the type of an existing sensor is a delete-and-recreate, not a `Set`, because it can silently drop type-specific fields.
...
**Why this exists now, and what it replaces.** An earlier hardening commit deliberately blocked any `Set` whose target was a whole table (a "container-Set") — without it, setting `sensors.new-id` to a JSON object would have silently minted a new entity with no id validation and no field restrictions. That block is still in effect; `Set` can never create or replace a whole entity table, and `type` on an already-existing entity still can't be changed. What's new is a dedicated create path — two patch operations, `create_entity` and `delete_entity` — that runs through its own gate instead of reusing (and thereby weakening) the `Set`/`Remove` gate. Creating an entity is now possible; smuggling one in through an unrestricted `Set` still isn't.
